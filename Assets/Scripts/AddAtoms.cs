...
public class AddAtoms : MonoBehaviour
{
    ...
    //Used to keep track of Gameobjects of ligands when several ligands in same PDB
    public bool SeveralLigands { get; set; }
    public List<bool> LigandsShowing { get; set; }
    public List<List<GameObject>> CollectionOfLigands { get; set; }
	...
	public void Awake ()
	{
        ...
        hetatmBS = PlayerPrefs.GetString("hetatmBS") == "True";
		...
		root = new GameObject ("molRoot");//a parent object that lets us move and rotate all objects as one
        ...
        parsePDB();
		setPosition (transform.position + transform.forward * 50);
        
        if (CollectionOfLigands != null)
        {
            LigandsShowing = new List<bool>();
            for (int i = 0; i < CollectionOfLigands.Count; i++)
            {
                LigandsShowing.Add(true);
                ToggleLigandDisplay(i, false);
            }
            ToggleLigandDisplay(0, true);
        }
    ...
    public void ToggleLigandDisplay(int index, bool value)
    {
        if (CollectionOfLigands != null)
        {
            LigandsShowing[index] = value;
            for (int i = 0; i < CollectionOfLigands[index].Count; i++)
            {
                CollectionOfLigands[index][i].SetActive(value);
            }
        }
    }
